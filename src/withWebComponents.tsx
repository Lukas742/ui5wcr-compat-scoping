...
>(
  ...
  return forwardRef<RefType, Props & WithWebComponentPropTypes>(
    (props, wcRef) => {
      ...
      const regularProps = regularProperties.reduce((acc, name) => {
        if (
          ...
          isPrimitiveAttribute(rest[name])
        ) {
        ...
        const removeFragments = (element: ReactNode) => {
          ...
          if (element.type === Fragment) {
            const elementChildren = (
              element as ReactElement<{ children?: ReactNode | ReactNode[] }>
            ).props?.children;
      ...
      const [attachEvents, setAttachEvents] = useState(
        !webComponentsSupported || !Object.keys(eventHandlers).length,
      ); // apply workaround only for React19 and if event props are defined
      ...
      useEffect(() => {
        if (waitForDefine && !isDefined) {
          void customElements
            .whenDefined(Component as unknown as string)
            .then(() => {
              setIsDefined(true);
      ...
      if (tagName === "ui5-expandable-text") {
        const renderWhiteSpace = nonWebComponentRelatedProps["renderWhitespace"]
          ? true
          : undefined;
        ...
        const showOverflowInPopover =
          nonWebComponentRelatedProps["showOverflowInPopover"];
        return (
          <Component
            ref={componentRef}
            {...booleanProps}
            ...
            overflow-mode={
              overflowMode ?? (showOverflowInPopover ? "Popover" : "InPlace")
            }
